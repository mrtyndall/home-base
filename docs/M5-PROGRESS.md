# Post-mortem: production stale-build audit (2026-07-03, evening)

- **Report:** the public Railway URL was reported externally as serving a pre-M5 nav (`Today/Tasks/Projects/Ideas/Search`) after M5 was merged.
- **Audit finding:** the Railway CLI link and the public domain owner were the same target: project `home-base` (`293a006f-f2d5-408d-abcb-8de2218be25f`), environment `production` (`4bea8124-cb21-4c56-83d3-7105aed019ff`), service `home-base` (`1dc07615-ae44-4dd1-b95b-6d85bac7a07b`), domain `home-base-production-e3b7.up.railway.app`. The service database host was `postgres.railway.internal`.
- **Root cause:** the "Railway CLI deployed to the wrong service/environment" hypothesis was false in this audit. Before the corrective redeploy, an external curl of `https://home-base-production-e3b7.up.railway.app` already returned the M5 shell with `Home` and `Library`; the stale external result could not be reproduced from the command line. The confirmed stale-origin risk remains the separate local/Tailscale LaunchAgent runtime and long-lived client instances, but this audit did not find a second Railway domain owner or database.
- **Fix applied:** redeployed exact commit `978a162` from a clean detached worktree to the domain-owning Railway service. New deployment `0aba6e7f-b007-438f-aabb-09a92cfa3c84` succeeded; deploy logs showed `npx prisma migrate deploy` ran before seed/server startup (`13 migrations found in prisma/migrations`, then `status="seeded"`, then `Ready`).
- **Verification:** external curl of `https://home-base-production-e3b7.up.railway.app` showed the bottom nav block with `Home`, `Today`, `Tasks`, `Projects`, and `Library`, and no bottom-nav `Search` label. `/settings` returned HTTP `200`.
- **Hardening:** AGENTS.md now names the exact Railway production project/service/environment/domain and requires raw external curl output in any future production-success report.
