...
Hermes and other MCP clients receive `HOME_BASE_MCP_URL` and `HOME_BASE_API_TOKEN` through approved environment references. `HOME_BASE_API_URL` belongs to the MCP server's REST upstream and the repository verifier, not the Hermes client. `npm run verify:agent-integration` allowlists the verified Home Base origins, checks local health, and, when a bearer reference exists, verifies MCP initialization, discovery, and representative reads. Its non-destructive capture/task write smoke requires the explicit `HOME_BASE_ENABLE_WRITE_SMOKE=1` opt-in, uses persistence-only idempotent capture, and cleans up every matching open smoke task by completion. See [the Hermes integration runbook](./docs/hermes-home-base-integration.md) for verified host identity, routes, 1Password-backed registration, secret-free configuration, and troubleshooting.
