...
assert.match(card, /ref={focusTargetRef}/, "The inbox card must expose a stable focus target.");
assert.match(card, /focusTargetRef\.current\?\.focus\(\)/, "Removal must move focus after its row unmounts.");
assert.match(card, /event\.channel === "schedule"[\s\S]{0,160}event\.phase === "optimistic"/, "Scheduling removal must request stable focus.");
assert.match(card, /pendingServer/, "Deferred reconciliation must retain the latest full canonical payload.");
assert.match(card, /if \(deferred\)[\s\S]{0,180}serverSignature:\s*current\.serverSignature/, "A deferred payload must not consume the applied server signature.");
