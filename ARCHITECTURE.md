...
For the nested-Area/optional-Project-Area and Read Later release, run `npm run verify:hierarchy-release -- --preflight` before migration and retain its Book, Movie, Area, Project, and Reference flags. After migration, run the same command with all five `--expected-*` values. Both phases execute in an explicitly read-only transaction and reject orphan Project Area references or Task/Idea/Reference Area mirrors that differ from their Project. Schema capability detection keeps the legacy preflight safe before `parent_area_id` and the Read Later columns or indexes exist: the count query does not reference absent columns and records zero only for unavailable checks. Strict postflight requires the hierarchy column, all four Read Later columns, the correctly defined named three-status check constraint, and the correctly defined named unique partial normalized-URL index. It also rejects Area cycles, orphan parents, duplicate active Read Later normalized URLs, invalid Read Later statuses, and baseline drift. Verification requires a successful rollback, reports problems, and never repairs data. Production preflight still requires a fresh backup, and migration application remains the deploy pipeline's responsibility.
...
- Read Later entries are References with `kind = read_later` and status `unread`, `read`, or `archived`. Opening is state-neutral; unread/read entries are active and unique by normalized URL, while archiving retains history and permits a later active save of the same URL.
...
The REST API lives under `/api/v1` and uses bearer API keys stored as hashes in `api_keys`. Minimum scopes are `read`, `write`, and `capture`; write rate limits are conservative by default. API routes intentionally expose no delete method. The hierarchy boundary is `GET/POST /areas`, `GET/PATCH /areas/:id`, `GET/POST /projects`, and `GET/PATCH /projects/:id`: Area reads include full paths, Area writes accept `parentAreaId`, and Project writes accept an optional or null `areaId`. The Read Later boundary lists/creates at `GET/POST /read-later`, reads at `GET /read-later/:id`, and changes state or filing through `POST /read-later/:id/status` and `POST /read-later/:id/file`; `POST /references/:id/file` retains the general Reference filing contract. Browser extensions and native share extensions are future REST clients, not release components.
...
Read Later reuses the Reference model with `kind = read_later`; Books and Movies remain distinct kinds and never appear in the reading queue. The submitted HTTP(S) URL is durable, while `normalized_url` removes common tracking noise for active-entry deduplication. `unread` is the default view, `read` preserves a finished entry, and `archived` removes it from the active queue without deleting it. `Open` never changes status. Explicit `read later <url>` capture language enters the queue, while a generic captured URL remains an ordinary Reference. Optional metadata enrichment cannot block the initial save, and entries can remain global or be filed/refiled to an Area or Project.
